...
exp_dir=./exp/demo-${model}-${eff_b}-${lr}-LJS-impretrain-${impretrain}-fm${freqm}-tm${timem}-mix${mixup}-bal-${bal}-b${batch_size}-le${p}-2
mkdir -p $exp_dir

CUDA_CACHE_DISABLE=1 python ../../src/run.py --data-train $trpath --data-val /vol/research/wenjieProject/projects/audio/dataset/test_data.json --data-eval /vol/research/wenjieProject/projects/audio/dataset/test_data.json \ 
